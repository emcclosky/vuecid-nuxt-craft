...
import { removeTrailingSlash, verifyLeadingSlash } from '@wearelucid/vuecid-helpers';

async function generateRoutesFromData(options = {
  endpoint: '',
  section: 'pages',
  token: '',
  homeSlug: 'home'
  ...
  const {
    ...
    endpoint,
    section
  } = options; // Fetch all sections with the specified section name from your GraphQL API

  let routes = await axios.post(endpoint, {
